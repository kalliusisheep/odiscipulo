...
const neutralizeCharacterPrompt = (prompt: string) =>
  prompt
    .replace(/\bQual mulher\b/gi, "Qual personagem")
    .replace(/\bQual profetisa\b/gi, "Qual personagem")
    .replace(/\bQual juíza\b/gi, "Qual personagem")
    .replace(/\bQual parteira\b/gi, "Qual personagem")
    .replace(/\bQual comandante\b/gi, "Qual personagem")
    .replace(/\bQual sacerdote\b/gi, "Qual personagem")
    .replace(/\bQual pregador\b/gi, "Qual personagem")
    .replace(/\bQual irmão\b/gi, "Qual personagem");
...
const FACT_CHARACTER_POOL: BiblicalCharacter[] = BIBLE_FACTS
  ...
  .map((fact) => ({
    id: "fact-character-" + fact.id,
    name: fact.answer,
    aliases: [fact.answer.toLocaleLowerCase("pt-BR")],
    difficulty: fact.difficulty,
    summary: maskCharacterAnswer(fact.statement, fact.answer),
    references: [fact.reference],
    hints: [
      neutralizeCharacterPrompt(fact.prompt).replace(/\?$/, ""),
      maskCharacterAnswer(fact.explanation, fact.answer),
      fact.completion.replace(/___/g, "este personagem"),
      maskCharacterAnswer(fact.statement, fact.answer),
    ],
  }));

const ALL_BIBLICAL_CHARACTERS = [...RAW_BIBLICAL_CHARACTERS, ...FACT_CHARACTER_POOL];
...
const isValidBiblicalCharacter = (character: BiblicalCharacter) => {
  ...
  return Boolean(character.id.trim() && character.name.trim() && character.summary.trim() && character.references.length > 0)
    ...
    && !hints.some((hint) => /examine|consta nos livros|consulte|sua aparição está registrada|é mencionado nas escrituras|narrativa bíblica apresenta/i.test(hint))
