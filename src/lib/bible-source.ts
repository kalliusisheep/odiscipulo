...
export type StrongEntry = {
  ...
  curated: boolean;
  /** Origem editorial da tradução exibida. */
...
const CORE_TERMS: Record<string, CoreTermOverride> = {
 ...
 …84 tokens truncated…o no NT (uso secundário)",
    ],
  },
  G3056: {
    original: "λόγος",
...
function buildStrongEntry(hit: BollsDictHit): StrongEntry {
  ...
  return {
    ...
    curated: false,
    translationStatus: "fonte-original",
    source: "Brown-Driver-Briggs / Thayer via bolls.life",
  };
}

type OpenPtLexiconRecord = {
  o: string | null;
  t: string | null;
  p: string | null;
  g: string;
...
const openPtBuckets = new Map<string, Promise<Record<string, OpenPtLexiconRecord>>>();

function openPtBucketFor(code: string): string | null {
  const match = code.toUpperCase().match(/^([GH])(\d+)$/);
  return match ? `${match[1]}${Math.floor(Number(match[2]) / 100)}` : null;
}

function loadOpenPtBucket(bucket: string): Promise<Record<string, OpenPtLexiconRecord>> {
  let promise = openPtBuckets.get(bucket);
  if (!promise) {
    promise = fetch(`/data/lexicon-pt/${bucket}.json`)
      .then((response) => (response.ok ? response.json() : {}))
      .catch(() => ({}));
    openPtBuckets.set(bucket, promise);
  }
  return promise;
}

async function fetchOpenPtEntry(code: string): Promise<StrongEntry | null> {
  const bucket = openPtBucketFor(code);
  if (!bucket) return null;
  const record = (await loadOpenPtBucket(bucket))[code];
  if (!record) return null;
  const definitions = record.d.filter(Boolean).slice(0, 3);
  return {
    code,
    original: record.o,
    transliteration: record.t,
    phonetic: record.p,
    partOfSpeech: translateGrammarTerms(record.g),
    origin: translateGrammarTerms(record.e),
    definitions,
    strongsGloss: definitions.join("; ") || null,
    meaning: buildMeaning(definitions[0] ?? null, definitions.slice(1), null),
    related: record.r,
    curated: false,
    translationStatus: "automatico",
    source: "BDB/Thayer · tradução offline aberta para português",
  };
}
  ...
  if (override) {
    return cached(cacheKey, async () => {
      ...
      return {
        ...
        source: "Verbete revisado manualmente a partir do léxico e do contexto bíblico",
      };
    });
  }

  return cached(cacheKey, async () => {
    const localEntry = await fetchOpenPtEntry(upperCode);
