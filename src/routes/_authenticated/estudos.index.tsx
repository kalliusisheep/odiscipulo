        ...
        <div className="mt-5 rounded-2xl border border-border bg-surface/80 p-1.5 shadow-sm">
          <div className="grid grid-cols-3 gap-1">
            {tabs.map((t) => {
              ...
              return (
                <button key={t.id} onClick={() => setTab(t.id)} className={`flex min-w-0 flex-col items-center justify-center gap-1.5 rounded-xl px-1 py-3 text-xs font-bold transition-all ${active ? "bg-primary text-primary-foreground shadow-md shadow-primary/20" : "text-muted-foreground hover:bg-surface-2 hover:text-foreground"}`}>
