...
export const Route = createFileRoute("/_authenticated/jogos/cruzadas")({
  validateSearch: (search: Record<string, unknown>) => ({
    ...
    rounds: normalizeRounds(search.rounds, 1),
  }),
  component: CrosswordPage,
});

type Direction = "across" | "down";
type Cell = { row: number; col: number; letter: string; wordIds: string[]; starts: Direction[] };
...
const roundOptions = [1, 3, 5, 10] as const;
...
function generatePuzzleAttempt(difficulty: GameDifficulty, theme: CrosswordTheme | "todos", seed?: number) {
  ...
  placements.forEach((placement) => placement.word.word.split("").forEach((letter, index) => {
    ...
    const cell = placedCells.get(cellKey) ?? { row, col, letter, wordIds: [], starts: [] };
    cell.wordIds.push(placement.word.id);
    if (index === 0) cell.starts.push(placement.direction);
    placedCells.set(cellKey, cell);
  }));
...
function CrosswordPage() {
  ...
  const selectCell = (cell: Cell) => {
    ...
    setActiveWordId((current) => {
      if (cell.wordIds.length < 2) return cell.wordIds[0];
      const currentIndex = current ? cell.wordIds.indexOf(current) : -1;
      return cell.wordIds[(currentIndex + 1) % cell.wordIds.length];
    });
    focusKeyboardInput();
  };
  ...
  const commitLetter = (rawLetter: string) => {
    if (!selectedCell || !activeWord) return;
    const cell = cellMap.get(selectedCell);
    if (!cell) return;
    const placement = puzzle.placements.find((item) => item.word.id === activeWord.id);
    if (!placement) return;

    const letter = rawLetter.toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    if (!/^[A-Z]$/.test(letter)) return;
    const expected = cell.letter.toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    ...
    if (letter !== expected) {
      ...
      window.setTimeout(() => setErrorCell(null), 350);
      return;
    }

    const nextLetters = { ...letters, [selectedCell]: letter };
    setLetters(nextLetters);
    updateCompleted(nextLetters);
    moveToNext(placement, cell.row, cell.col);
  };
  ...
  const handleKey = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    event.preventDefault();
    commitLetter(event.key);
  };

  const handleInput = (event: FormEvent<HTMLInputElement>) => {
    const rawValue = event.currentTarget.value;
    event.currentTarget.value = "";
    const letter = rawValue.match(/[a-zA-ZÀ-ÿ]/)?.[0];
  ...
  if (phase === "setup" && !modeSelected) return <GameModeChooser title="Palavras Cruzadas" description="Escolha uma grade para resolver sozinho ou reúna seus amigos para um desafio bíblico em sala." onBack={() => { window.location.href = "/jogos"; }} onSinglePlayer={() => setModeSelected(true)} onMultiplayer={() => { window.location.href = "/jogos/multiplayer?game=cruzadas"; }} />;
  ...
  if (phase === "finished") return <Results score={score} seconds={seconds} errors={errors} hints={hints} rounds={rounds} placements={puzzle.placements} onRestart={() => setPhase("setup")} />;
  ...
  return <main className="game-play-page min-h-screen bg-background"><div className="mx-auto max-w-lg px-3 pb-28 pt-5"><header className="flex items-center justify-between"><Link to="/jogos" className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground"><ArrowLeft className="h-4 w-4" /> Jogos</Link><span className="flex items-center gap-2 rounded-full bg-surface px-3 py-1.5 text-xs font-extrabold"><Clock3 className="h-3.5 w-3.5 text-primary" /><span role="timer" aria-live="polite" aria-atomic="true">{Math.floor(seconds / 60)}:{String(seconds % 60).padStart(2, "0")}</span></span></header><div className="mt-6 flex items-end justify-between"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-primary">Palavras Cruzadas</p><h1 className="mt-1 text-2xl font-extrabold">Descubra a Palavra</h1></div><div className="text-right"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Pontos</p><p className="text-xl font-black text-ancient">{score}</p></div></div><div className="mt-4 flex items-center justify-between text-[11px] font-bold text-muted-foreground"><span>{completed.length}/{puzzle.placements.length} palavras completas</span><span className="inline-flex items-center gap-1 text-ancient"><Flame className="h-3.5 w-3.5" /> {config.label}</span></div><section className="mt-4 rounded-[1.75rem] border border-primary/20 bg-surface p-3 shadow-xl shadow-primary/5"><div className="grid gap-0.5" style={{ gridTemplateColumns: `repeat(${puzzle.size}, minmax(0, 1fr))` }}>{gridCells.map((cell, index) => { if (!cell) return <span key={index} className="aspect-square rounded-[4px] border border-border/35 bg-background/45" aria-hidden="true" />; const cellKey = key(cell.row, cell.col); const isActive = activeWordId ? cell.wordIds.includes(activeWordId) : false; const isSelected = cellKey === selectedCell; return <button key={cellKey} type="button" onClick={() => selectCell(cell)} aria-label={`Célula ${cell.row + 1}, ${cell.col + 1}`} className={`relative aspect-square rounded-[4px] border text-[clamp(0.58rem,3.4vw,1rem)] font-black transition-all ${isSelected ? "z-10 border-primary bg-primary text-primary-foreground shadow-[0_0_0_3px_hsl(var(--primary)/0.3)]" : isActive ? "border-primary/60 bg-primary/15 text-primary" : "border-border bg-surface-2/80 text-foreground"} ${errorCell === cellKey ? "animate-pulse border-red-400 bg-red-400/20 text-red-300" : ""}`}>{cell.starts.length > 0 && <span className="pointer-events-none absolute left-0.5 top-0 text-[8px] leading-none text-primary/80">{cell.starts.includes("across") ? "→" : <ArrowDown className="h-2 w-2" />}</span>}{letters[cellKey] ?? ""}</button>; })}</div></section><input ref={inputRef} type="text" inputMode="text" maxLength={1} onKeyDown={handleKey} onInput={handleInput} className="fixed left-1 top-1 z-10 h-px w-px opacity-0" aria-label="Digite a letra da célula selecionada" autoCapitalize="characters" autoCorrect="off" spellCheck={false} autoComplete="off" /><section className="mt-4 rounded-3xl border border-primary/20 bg-primary/5 p-4"><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Sparkles className="h-5 w-5" /></span><div className="min-w-0 flex-1"><p className="text-[10px] font-extrabold uppercase tracking-wider text-primary">{activeWord?.category ?? "Selecione uma palavra"}</p><p className="mt-1 text-sm font-extrabold leading-relaxed">{activeWord?.clue ?? "Toque em uma célula para começar."}</p><p className="mt-1 text-[11px] text-muted-foreground">{activeWord ? `${activeWord.word.length} letras · ${activePlacement?.direction === "down" ? "vertical" : "horizontal"}` : ""}</p></div><button type="button" onClick={revealHint} disabled={!activeWord} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-ancient/30 bg-ancient/10 text-ancient disabled:opacity-40" aria-label="Usar dica"><Lightbulb className="h-4 w-4" /></button></div>{activeWord && <p className="mt-3 text-[11px] leading-relaxed text-muted-foreground">Dica: revela uma letra e custa 25 pontos. Referência: <span className="font-bold text-foreground">{activeWord.reference}</span></p>}</section><div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-2xl border border-border bg-surface p-3"><p className="text-[10px] text-muted-foreground">Erros</p><p className="mt-1 text-lg font-black text-red-300">{errors}</p></div><div className="rounded-2xl border border-border bg-surface p-3"><p className="text-[10px] text-muted-foreground">Dicas usadas</p><p className="mt-1 text-lg font-black text-ancient">{hints}</p></div></div></div></main>;
}

function Setup({ difficulty, setDifficulty, theme, setTheme, rounds, setRounds, onStart }: { difficulty: GameDifficulty; setDifficulty: (value: GameDifficulty) => void; theme: CrosswordTheme | "todos"; setTheme: (value: CrosswordTheme | "todos") => void; rounds: number; setRounds: (value: number) => void; onStart: () => void }) {
  return <main className="game-play-page min-h-screen bg-background"><div className="mx-auto max-w-lg px-4 pb-28 pt-5"><Link to="/jogos" className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground"><ArrowLeft className="h-4 w-4" /> Jogos</Link><header className="mt-8"><p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-primary">Desafio de descoberta</p><h1 className="mt-1 text-3xl font-extrabold">Palavras Cruzadas</h1><p className="mt-3 text-sm leading-relaxed text-muted-foreground">Preencha a grade com palavras da Bíblia. Toque, digite e deixe cada cruzamento ensinar algo novo.</p></header><section className="mt-8 space-y-6 rounded-[1.75rem] border border-border bg-surface p-5"><div><p className="text-xs font-extrabold uppercase tracking-wider text-muted-foreground">Personalidade da grade</p><div className="mt-3 grid grid-cols-2 gap-2">{difficulties.map((option) => <button key={option} type="button" onClick={() => setDifficulty(option)} className={`rounded-2xl border px-3 py-3 text-left ${difficulty === option ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground"}`}><span className="block text-sm font-extrabold">{CROSSWORD_DIFFICULTY[option].label}</span><span className="mt-1 block text-[10px] leading-snug opacity-75">{CROSSWORD_DIFFICULTY[option].description}</span></button>)}</div></div><label className="block text-xs font-extrabold uppercase tracking-wider text-muted-foreground">Tema<select value={theme} onChange={(event) => setTheme(event.target.value as CrosswordTheme | "todos")} className="mt-2 w-full rounded-xl border border-border bg-background px-3 py-3 text-sm font-bold text-foreground"><option value="todos">Todos os temas</option>{CROSSWORD_THEMES.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label><div><p className="text-xs font-extrabold uppercase tracking-wider text-muted-foreground">Número de rodadas</p><div className="mt-3 grid grid-cols-4 gap-2">{roundOptions.map((option) => <button type="button" key={option} onClick={() => setRounds(option)} className={`rounded-2xl border py-3 text-sm font-extrabold ${rounds === option ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground"}`}>{option}<span className="mt-1 block text-[9px] font-medium opacity-70">{option === 1 ? "uma grade" : "rodadas"}</span></button>)}</div></div><button type="button" onClick={onStart} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-primary px-4 py-4 text-sm font-extrabold text-primary-foreground shadow-lg shadow-primary/20">Montar minha grade <ArrowRight className="h-4 w-4" /></button></section></div></main>;
}

function Results({ score, seconds, errors, hints, rounds, placements, onRestart }: { score: number; seconds: number; errors: number; hints: number; rounds: number; placements: Placement[]; onRestart: () => void }) {
  return <main className="game-play-page min-h-screen bg-background"><div className="mx-auto max-w-lg px-4 pb-28 pt-10 text-center"><span className="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl bg-ancient/15 text-ancient"><Trophy className="h-10 w-10" /></span><p className="mt-7 text-[10px] font-extrabold uppercase tracking-[0.2em] text-primary">Grade concluída</p><h1 className="mt-2 text-3xl font-extrabold">Você dominou a grade!</h1><section className="mt-7 rounded-[1.75rem] border border-border bg-surface p-6"><p className="text-xs font-extrabold uppercase tracking-wider text-muted-foreground">Pontuação final</p><p className="mt-2 text-5xl font-black text-ancient">{score}</p><p className="mt-2 text-xs text-muted-foreground">{rounds} {rounds === 1 ? "grade concluída" : "grades concluídas"}</p><div className="mt-6 grid grid-cols-3 gap-2"><div><p className="text-xl font-black">{Math.floor(seconds / 60)}:{String(seconds % 60).padStart(2, "0")}</p><p className="text-[10px] text-muted-foreground">tempo</p></div><div><p className="text-xl font-black">{errors}</p><p className="text-[10px] text-muted-foreground">erros</p></div><div><p className="text-xl font-black">{hints}</p><p className="text-[10px] text-muted-foreground">dicas</p></div></div></section><section className="mt-4 space-y-2 text-left"><p className="px-1 text-xs font-extrabold uppercase tracking-wider text-muted-foreground">Continue estudando</p>{placements.map((placement) => <div key={placement.word.id} className="rounded-2xl border border-border bg-surface p-3"><p className="text-sm font-extrabold">{placement.word.word}</p><p className="mt-1 text-xs text-muted-foreground">{placement.word.curiosity}</p><p className="mt-2 text-[10px] font-bold text-primary">{placement.word.reference}</p></div>)}</section><div className="mt-6 flex gap-3"><a href="/jogos/cruzadas" className="flex flex-1 items-center justify-center gap-2 rounded-2xl border border-border px-4 py-3 text-sm font-bold"><ArrowLeft className="h-4 w-4" /> Tela inicial</a><button type="button" onClick={onRestart} className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-primary px-4 py-3 text-sm font-extrabold text-primary-foreground"><RotateCcw className="h-4 w-4" /> Jogar de novo</button></div></div></main>;
}
