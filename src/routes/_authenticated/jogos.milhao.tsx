...
function Setup({
  ...
  onDifficultyChange: (value: MillionDifficulty) => void;
  rounds: number;
  onRoundsChange: (value: number) => void;
  onStart: () => void;
}) {
  return (
    <main className="game-play-page min-h-[100dvh] bg-background">
      <div className="mx-auto flex min-h-[calc(100dvh-4rem)] max-w-lg flex-col px-3 pb-20 pt-3">
        <Link
          to="/jogos"
        ...
        <header className="mt-4">
          ...
          <h1 className="mt-1 text-2xl font-extrabold leading-tight">Quiz do Milhão</h1>
          <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-muted-foreground">
            Suba pelos títulos do conhecimento bíblico, preserve seu combo e enfrente o Desafio Bereano.
          </p>
        </header>

        <section className="mt-4 rounded-[1.5rem] border border-border bg-surface p-3.5">
          <p className="text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground">
            Escolha seu nível
          </p>

          <div className="mt-2 grid grid-cols-2 gap-1.5">
            {(Object.entries(MILLION_DIFFICULTY) as [
              MillionDifficulty,
              ...
              <button
                ...
                className={
                  difficulty === key
                    ? "min-h-[62px] rounded-2xl border border-primary bg-primary/10 px-2.5 py-2 text-left text-primary shadow-sm shadow-primary/10 transition-all"
                    : "min-h-[62px] rounded-2xl border border-border bg-background/40 px-2.5 py-2 text-left transition-all"
                }
              >
                <span className="block text-xs font-extrabold leading-tight">{item.label}</span>
                <span className="mt-1 block line-clamp-2 text-[9px] leading-snug text-muted-foreground">
                  {item.description}
                </span>
              </button>
            ))}
          ...
          <div className="mt-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold text-primary">{rounds} rodadas</span>
            </div>
            <div className="mt-2 grid grid-cols-5 gap-1.5">
              {roundOptions.map((option) => (
                <button
                  ...
                  className={
                    rounds === option
                      ? "rounded-xl border border-primary bg-primary/10 py-2.5 text-xs font-extrabold text-primary"
                      : "rounded-xl border border-border bg-background/40 py-2.5 text-xs font-extrabold text-muted-foreground"
                  }
                >
                  {option}
                </button>
          ...
          <button
            type="button"
            onClick={onStart}
            className="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl bg-primary px-4 py-3 text-sm font-extrabold text-primary-foreground shadow-lg shadow-primary/20"
          >
