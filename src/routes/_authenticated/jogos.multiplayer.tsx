...
function MultiplayerPage() {
  ...
  const loadRoom = async (nextRoomId: string, userId: string) => {
    const [{ data: nextRoom, error: roomError }, { data: roomPlayers, error: playersError }] = await Promise.all([
    ...
    if (roomError || playersError || !nextRoom) {
      setLoading(false);
      setSyncError("Não foi possível sincronizar esta sala. Verifique se as migrations de partidas foram aplicadas no Supabase e tente novamente.");
      return;
    }
    setSyncError(null);
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      if (targetRoomId) {
        const refreshRoom = () => {
          if (!cancelled) void loadRoom(targetRoomId, data.user.id);
        };
        await loadRoom(targetRoomId, data.user.id);
        if (cancelled) return;
        channel = supabase
          .channel(`game-room-${targetRoomId}`)
          .on("postgres_changes", { event: "*", schema: "public", table: "character_game_room_players", filter: `room_id=eq.${targetRoomId}` }, refreshRoom)
          .on("postgres_changes", { event: "UPDATE", schema: "public", table: "character_game_rooms", filter: `id=eq.${targetRoomId}` }, refreshRoom)
          .subscribe((status) => {
            if (status === "CHANNEL_ERROR" || status === "TIMED_OUT") refreshRoom();
          });
        poll = window.setInterval(refreshRoom, 5000);
      }
    };
    void boot();
    return () => {
  ...
  if (syncError && !room) return <main className="min-h-screen bg-background"><div className="mx-auto flex min-h-screen max-w-lg items-center px-4 pb-28"><section className="w-full rounded-[1.75rem] border border-red-400/30 bg-surface p-6 text-center"><Wifi className="mx-auto h-10 w-10 text-red-300" /><h1 className="mt-4 text-xl font-extrabold">Sala não sincronizada</h1><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{syncError}</p><Link to="/jogos" className="mt-5 inline-flex items-center gap-2 rounded-2xl bg-primary px-4 py-3 text-sm font-extrabold text-primary-foreground"><ArrowLeft className="h-4 w-4" /> Voltar aos jogos</Link></section></div></main>;
